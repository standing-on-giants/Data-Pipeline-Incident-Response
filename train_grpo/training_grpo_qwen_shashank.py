# -*- coding: utf-8 -*-
"""training-grpo-qwen.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/github/standing-on-giants/Meta_hackathon/blob/dev%2Fpratham/train_grpo/training-grpo-qwen.ipynb
...
GOLD_ACTIONS = {
    ...
    # REPLACE the 'hard' and 'hard2' entries in GOLD_ACTIONS with:
    'hard': [
...
# The reconstruction actions — same order used when building the dataset
# so the reward function can replay them to get back to the right env state
...
def pipeline_reward_fn(completions, task_id=None, n_prior_actions=None, **kwargs):
    ...
    # TRL passes dataset columns as parallel lists
    ...
    for c, tid, n_prior in zip(completions, task_ids, priors):
        ...
        try:
            ...
            # NOW evaluate the model's action in the correct context
            result = env.step(action)
            reward += result.reward or 0.0
            obs_after = result.observation

            # Bonus for actually improving assertions
...
# Sanity check
test_r = pipeline_reward_fn(
    ['{"action_type": "read_data_sample", "params": {"table": "raw_ads_insights", "n_rows": 10}}'],
    task_id=['hard'], n_prior_actions=[0]
)
print(f'Reward sanity check: {test_r} (should be > 0.3)')

# Build dataset with BOTH step-1 and step-2/3 prompts
# so the model learns correct behavior at each episode position
...
print(f'  Step distribution: {grpo_ds["n_prior_actions"].count(0)} step-1, '
      f'{grpo_ds["n_prior_actions"].count(1)} step-2, '
      f'{grpo_ds["n_prior_actions"].count(2)} step-3')
...
print(f'Dataset size: {len(grpo_ds)} — includes step-1, step-2, step-3 prompts')
